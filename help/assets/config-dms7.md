...
1. Wanneer het primaire bronelement naar AEM wordt geüpload, wordt het naar Dynamic Media gerepliceerd. Op dat moment verwerkt Dynamic Media alle processen voor het genereren van elementen, zoals videocodering en dynamische varianten van een afbeelding. <!-- (In Dynamic Media - Scene7 mode, be aware that you can only upload assets whose file sizes are 2 GB or less.) Jira ticket CQ-4286561 fixed this issue. DM-S7 NOW SUPPORTS THE UPLOAD OF ASSETS LARGER THAN 2 GB. -->
...
>[!NOTE]
>
>De modus Dynamic Media - Scene7 is alleen bedoeld voor de instantie AEM Author. Als dusdanig, moet u `runmode=dynamicmedia_scene7` op de instantie van de Auteur van AEM, *not* de instantie van de Publicatie van AEM vormen.
...
Als u een upgrade uitvoert AEM Dynamic Media van 6.3 naar 6.4 of 6.5 (die nu de mogelijkheid biedt om downtime nul te implementeren), moet u de volgende curl-opdracht uitvoeren om al uw voorinstellingen en configuraties te migreren van `/etc` naar `/conf` in CRXDE Lite.
...
Met de Feature Pack 18912 kunt u middelen bulksgewijs invoeren via FTP, of middelen migreren van Dynamic Media - Hybride modus of Dynamic Media Classic naar Dynamic Media - Scene7 modus op AEM. Het is beschikbaar bij [Adobe Professional Services](https://www.adobe.com/experience-cloud/consulting-services.html).
...
1. Wanneer de verbinding is gelukt, stelt u het volgende in. Koppen met een sterretje (*) zijn vereist:
   ...
   * **[!UICONTROL Publishing Assets]** - U kunt uit de volgende drie opties kiezen:
      ...
      * **[!UICONTROL Selective Publish]** Met deze opties kunt u bepalen welke mappen in Dynamic Media worden gepubliceerd, zodat u functies kunt gebruiken, zoals SmartCrop of dynamische uitvoeringen of mappen die alleen in AEM worden gepubliceerd voor voorvertoningen. dezelfde activa worden in Dynamic Media  ** niet gepubliceerd voor levering in het publieke domein.<br>U kunt deze optie hier in de map instellen  **[!UICONTROL Dynamic Media Cloud Configuration]** of, als u dat wilt, u kunt deze optie instellen op mapniveau, in de map  **[!UICONTROL Properties]**.<br>Zie  [Werken met Selectieve publicatie in Dynamic Media.](/help/assets/selective-publishing.md)<br>Als u deze configuratie later wijzigt of deze later wijzigt op mapniveau, hebben die wijzigingen alleen invloed op nieuwe elementen die u vanaf dat punt uploadt. De publicatiestatus van bestaande elementen in de map blijft ongewijzigd totdat u deze handmatig wijzigt vanuit **[!UICONTROL Quick Publish]** of het dialoogvenster **[!UICONTROL Manage Publication]**.
...
Als u een geselecteerde map voor synchronisatie met Dynamic Media wilt markeren, selecteert u een elementmap en klikt u op **[!UICONTROL Properties.]** op het tabblad **[!UICONTROL Details]** in de vervolgkeuzelijst **[!UICONTROL Dynamic Media sync mode]** op de werkbalk. Kies vervolgens een van de volgende drie opties. Wanneer u klaar bent, tikt u **[!UICONTROL Save.]** *Onthoud: Deze drie opties zijn niet beschikbaar als u **Alle inhoud synchroniseren**eerder hebt geselecteerd.* Zie ook  [Werken met Selectief publiceren op mapniveau in Dynamic Media.](/help/assets/selective-publishing.md)
...
Als u in de runmode `dynamicmedia_scene7` bent, gebruikt u de Dynamic Media Classic (Scene7)-gebruikersinterface om uw Dynamic Media-instellingen te wijzigen.
