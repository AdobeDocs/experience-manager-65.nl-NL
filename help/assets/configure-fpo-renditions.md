...
description: Genereer FPO-uitvoeringen van nieuwe en bestaande elementen met behulp van AEM workflow en ImageMagick.
contentOwner: Vishabh Gupta
role: Admin
feature: FPO-uitvoeringen, uitvoeringen
exl-id: null
...
In AEM wordt de workflow voor DAM Update Asset uitgevoerd wanneer een nieuw element wordt geüpload. Als u ImageMagick wilt gebruiken om uitvoeringen van nieuw geüploade elementen te verwerken, voegt u een nieuwe opdracht toe aan het workflowmodel.
...
## Uitvoeringen van bestaande elementen genereren met AEM workflow {#generate-renditions-of-existing-assets-using-aem-workflow}

Als u AEM workflow wilt gebruiken om FPO-uitvoering van de bestaande elementen te genereren, maakt u een speciaal workflowmodel dat gebruikmaakt van de ingebouwde optie voor FPO-uitvoering.
...
1. Selecteer **[!UICONTROL Process Thumbnails]** en klik **[!UICONTROL Configure]**. Volg de [configuratie om vertoning van nieuwe activa te produceren gebruikend AEM werkschema](#generate-renditions-of-new-assets-using-aem-workflow). Klik op **[!UICONTROL Sync]** om de wijzigingen te activeren.
...
1. Volg stap 1 tot stap 3 van [configuratie om vertoning van bestaande activa te produceren gebruikend AEM werkschema](#generate-renditions-of-existing-assets-using-aem-workflow) sectie.
...
U kunt de gegenereerde FPO-uitvoeringen controleren nadat de workflow is voltooid. Klik in de AEM Assets-gebruikersinterface op het element om een grote voorvertoning te openen. Open het linkerspoor en selecteer Vertoningen. U kunt ook de sneltoets `Alt + 3` gebruiken wanneer de voorvertoning is geopend.
...
* Om op beeldMagick-Gebaseerde configuratie te gebruiken, installeer ImageMagick op de zelfde machine zoals AEM.
* Om FPO vertoningen van vele activa of van de volledige bewaarplaats te produceren, plan en voer de werkschema&#39;s tijdens laag-verkeersduur uit. Het genereren van FPO-uitvoeringen voor een groot aantal elementen is een activiteit die veel resources vereist en de AEM servers moeten over voldoende verwerkingskracht en geheugen beschikken.
* Voor prestaties en schaalbaarheid, zie [Fine-tune ImageMagick](https://experienceleague.adobe.com/docs/experience-manager-65/assets/administer/performance-tuning-guidelines.html).
* Zie [opdrachtregelhandler om elementen te verwerken](https://experienceleague.adobe.com/docs/experience-manager-65/assets/extending/media-handlers.html) voor algemene opdrachtregelafhandeling van elementen.