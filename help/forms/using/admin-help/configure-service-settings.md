...
1. Klik op het tabblad Beveiliging en stel de beveiligingsinstellingen voor de service in. Zie [Beveiligingsinstellingen voor een service wijzigen](configure-service-settings.md#modifying-security-settings-for-a-service).
...
De service voor streepjescodes `(BarcodedFormsService)` Hiermee haalt u streepjescodegegevens uit gescande afbeeldingen. De service accepteert een formulier met streepjescodes (TIFF of PDF) als invoer en extraheert de machinale weergave van de gegevens die door de streepjescode zijn gecodeerd.
...
**Basisbestandspad:** Het bestandspad waarvoor de parameters voor invoer- en uitvoerbestanden voor de bewerkingen XML-bestandstaak uitvoeren en Platte bestandstaak uitvoeren zijn opgelost. In gegroepeerde configuraties, moet de weg van het basisdossier een gedeelde plaats zijn van het dossiersysteem waaraan alle clusterknopen lees-schrijftoegang hebben.
...
De Content Repository Connector voor EMC Documentum Service ( `EMCDocumentumContentRepositoryConnector`) kunt u processen maken die interageren met inhoud die is opgeslagen in een Documentum repository.
...
Als het standaardpad bijvoorbeeld is ingesteld op `/LiveCycleES/ConnectorforEMCDocumentum/AssetLinkObjects`en de formuliersjabloon wordt opgeslagen in een map `/Docbase/forms/`, wordt het object Asset Link opgeslagen op de volgende locatie:
...
Als het standaardpad bijvoorbeeld is ingesteld op `/LiveCycleES/ConnectorforIBMFileNet/AssetLinkObjects`en de formuliersjabloon wordt opgeslagen in een map `/Docbase/forms/`, wordt het object Asset Link opgeslagen op de volgende locatie:
...
E-mail wordt doorgaans gebruikt om inhoud te verspreiden of statusinformatie te verstrekken als onderdeel van een geautomatiseerd proces. De e-mailservice ( `EmailService`) laat processen toe om e-mailberichten van een POP3 of server te ontvangen IMAP, en e-mailberichten naar een server te verzenden SMTP.
...
**Poolgrootte van ImageToPDF:** De groepsgrootte van de standaard (zuivere Java) converter van afbeelding naar PDF in de service PDF genereren. Met deze instelling bepaalt u de maximale gelijktijdige omzetting van afbeelding in PDF die de service PDF genereren kan uitvoeren. De standaardwaarde van deze instelling (aanbevolen voor systemen met één processor) is 3, die u kunt verhogen voor systemen met meerdere processors.
...
**macAlgorithm:** Het MAC-algoritme (berichtverificatiecode) dat moet worden gebruikt voor URL-verificatie van onderpand. Deze methode wordt doorgegeven aan de methode getInstance van de klasse Mac. Standaard is HmacSHA1.
...
De JDBC-service ( `JdbcService`) laat processen toe om met gegevensbestanden in wisselwerking te staan.
...
De JMS-service ( `JMS`) maakt interactie met JMS-providers (Java Messaging System) mogelijk die zowel point-to-point berichten als berichten publiceren/abonneren implementeren.
...
**Seconden van taakvervaldatum:** Met de service PDF genereren verwijdert u invoerbestanden zodra deze zijn geconverteerd. De uitvoerbestanden worden tijdelijk opgeslagen, gedurende een periode die wordt bepaald door de instellingen Seconden van PDFG-opschoning en Seconden voor taakvervaldatum.
...
De standaardwaarde voor de PDFG Cleanup Scan Seconds is `43200` (12 uur). De standaardwaarde van Seconden van de Verlopen van de Baan is `86400` (24 uur).

**Standaardlandinstelling:** Wordt gebruikt om de standaardlandinstelling (land + taal) van de server te overschrijven waar de service PDF genereren is geïmplementeerd. Als deze parameter niet gespecificeerd is, dan wordt de standaardscène bepaald van het werkende systeem waarop de dienst wordt opgesteld. Deze parameter bestuurt de taal waarin de foutenmeldingen aan APIs zijn teruggekeerd.
...
**Documenten verwerken met Acrobat 9-compatibiliteit:** Geeft aan of Acrobat 9-compatibiliteit moet worden ingeschakeld. Als deze optie bijvoorbeeld is ingeschakeld, wordt Zichtbare certificering in dynamische PDF ingeschakeld. De standaardwaarde is false.
...
**Intrekkingsgegevens insluiten tijdens certificering:** Hiermee wordt opgegeven of de intrekkingsinformatie wordt ingesloten tijdens de certificering van het PDF-document. De standaardwaarde is false.

**Insluiten van informatie over intrekking afdwingen voor alle certificaten tijdens ondertekening/certificering:** Geeft aan of een ondertekenings- of certificeringsbewerking mislukt als geldige intrekkingsgegevens voor alle certificaten niet zijn ingesloten. Als een certificaat geen CRL- of OCSP-informatie bevat, wordt het als geldig beschouwd, zelfs als er geen intrekkingsinformatie wordt opgehaald. De standaardwaarde is false.
...
**Gebruikersnaam TSP-server:** De gebruikersnaam indien nodig door de tijdstempelprovider. Wordt alleen gebruikt als er een geldige waarde voor de URL is opgegeven. Geen standaardwaarde.

**Wachtwoord TSP-server:** Het wachtwoord voor de bovenstaande gebruikersnaam indien nodig door de tijdstempelprovider. Wordt alleen gebruikt als er een geldige waarde voor de URL en de gebruikersnaam is opgegeven. Geen standaardwaarde.
...
De service Gecontroleerde map ( `WatchedFolder`) configureert kenmerken die algemeen gelden voor alle gecontroleerde eindpunten van mappen. De klasse biedt ook standaardwaarden voor gecontroleerde eindpunten van mappen. (Zie [Gecontroleerde eindpunten van mappen configureren](/help/forms/using/admin-help/configuring-watched-folder-endpoints.md#configuring-watched-folder-endpoints).) Het wordt niet aangehaald door externe cliënttoepassingen of gebruikt in processen die in Workbench worden gecreeerd.
...
Als het pad niet absoluut maar relatief is, wordt de map in de controlemap gemaakt. Zie voor meer informatie over bestandspatronen [Bestandspatronen](/help/forms/using/admin-help/configuring-watched-folder-endpoints.md#about-file-patterns).
...
De dienst van de Dienst van het Web ( `WebService`) biedt processen de mogelijkheid om webservicebewerkingen aan te roepen.
...
De XSLT Transformation Service ( `XSLTService`) biedt processen de mogelijkheid om XSLT (Extensible Stylesheet Language Transformations) toe te passen op XML-documenten.
