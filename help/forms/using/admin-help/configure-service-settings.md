...
1. Stop de service voordat u deze wijzigt. (Zie [Services starten en stoppen](/help/forms/using/admin-help/starting-stopping-services.md#starting-and-stopping-services).)
...
**Links lezen:** Wanneer deze optie is geselecteerd, worden streepjescodeafbeeldingen horizontaal van rechts naar links gescand.

**Rechts lezen:** Wanneer deze optie is ingeschakeld, worden streepjescodeafbeeldingen horizontaal van links naar rechts gescand.

**Omhoog lezen:** als deze optie is ingeschakeld, worden streepjescodeafbeeldingen verticaal van beneden naar boven gescand.

**Omlaag lezen:** Wanneer deze optie is ingeschakeld, worden streepjescodeafbeeldingen verticaal van boven naar beneden gescand.
...
**Basisbestandspad:** het bestandspad waarvoor de parameters voor invoer- en uitvoerbestanden van de batch voor de bewerkingen XML-bestandtaak uitvoeren en Platte bestandstaken uitvoeren zijn opgelost. In gegroepeerde configuraties, moet de weg van het basisdossier een gedeelde plaats zijn van het dossiersysteem waaraan alle clusterknopen lees-schrijftoegang hebben.
...
**Standaardpad voor Asset Link Object:** het standaardgedeelte van het pad in de Documentum-opslagplaats voor de opslag van het Asset Link-object. Het feitelijke pad bestaat uit het standaardpad en de locatie van de formuliersjabloon in de opslagplaats voor AEM formulieren.
...
**Standaardpad voor Object Asset Link:** het standaardgedeelte van het pad in de IBM FileNet-opslagruimte voor het opslaan van het Asset Link-object. Het feitelijke pad bestaat uit het standaardpad en de locatie van de formuliersjabloon in de opslagplaats voor AEM formulieren.
...
**Vereist Nieuw:** maakt altijd een nieuwe transactiecontext. Als er een actieve transactiecontext bestaat, wordt deze opgeschort.

**De Tijd van de transactie uit (in seconden):** het aantal seconden dat de onderliggende transactieleverancier zou moeten wachten alvorens een transactie terug te rollen die deze verrichting verpakt. Deze waarde wordt genegeerd als een bestaande transactiecontext wordt doorgegeven. De standaardwaarde is 180.

**Drempelresolutie voor vloeiend maken (in dpi):** De afbeeldingsresolutie waaronder vloeiend maken (of anti-aliasing) wordt toegepast op tekst, lijntekeningen en afbeeldingen, als u de optie Vloeiend maken toepassen op hebt geselecteerd voor deze elementen.

**Vloeiend maken op tekst toepassen:** regelt anti-aliasing van tekst. Schakel dit selectievakje uit als u het vloeiend maken van tekst wilt uitschakelen en tekst scherper en beter leesbaar wilt maken met een schermvergroting.

**Vloeiend maken toepassen op LineArt:** Hiermee past u vloeiend maken toe om abrupte hoeken in lijnen te verwijderen.

**Vloeiend maken toepassen op afbeeldingen:** Hiermee past u vloeiend maken toe om abrupte wijzigingen in afbeeldingen te minimaliseren.

## Distiller-service-instellingen {#distiller-service-settings}

Met de Distiller-service ( `DistillerService`) worden PostScript-, Encapsulated PostScript- (EPS) en PRN-bestanden via een netwerk geconverteerd naar PDF-bestanden.

De volgende instellingen zijn beschikbaar voor de Distiller-service.

**Adobe PDF-instellingen:** de volgende vooraf geconfigureerde instellingen worden toegepast op het gegenereerde PDF-bestand:
...
**Poolgrootte:** de oorspronkelijke grootte van de pool. Wanneer de dienst van Distiller wordt opgesteld, wordt dit aantal gebruikt om het aantal instanties van de de dienstimplementatie te bepalen die worden gecreeerd en aan de vrije pool toegewezen wachtend op aanroepingsverzoeken. De de dienstcontainer kan dan onmiddellijk aan aanroepingsverzoeken antwoorden zonder het moeten een de dienstinstantie eerst initialiseren.
...
**SMTP verifieert:** selecteer als de gebruikersauthentificatie wordt vereist om met de server te verbinden SMTP.

**SMTP-gebruiker:** De gebruikersnaam van de gebruikersaccount die moet worden gebruikt voor aanmelding bij de SMTP-server.

**SMTP-wachtwoord:** het wachtwoord dat aan de SMTP-gebruikersaccount is gekoppeld.

**SMTP-transportbeveiliging:** het beveiligingsprotocol dat moet worden gebruikt voor verbinding met de SMTP-server:
...
**POP3/IMAP-gebruikersnaam:** de gebruikersnaam van de gebruikersaccount die moet worden gebruikt om zich aan te melden bij de POP3- of IMAP-server.

**POP3/IMAP-wachtwoord:** het wachtwoord dat aan de POP3- of IMAP-gebruikersaccount is gekoppeld.
...
**Ontvang de Veiligheid van het Vervoer:** het veiligheidsprotocol voor het verbinden met de server SMTP te gebruiken:
...
**Standaard LDAP-server voor verbinding met:** hostnaam van de LDAP-server waarmee certificaten voor documentversleuteling worden opgehaald.

**Standaard LDAP-poort voor verbinding met:** poortnummer van de LDAP-server.

**Standaard LDAP-gebruikersnaam:** als verificatie voor de LDAP-server is vereist, geeft u de gebruikersnaam op die moet worden gebruikt om verbinding te maken met de LDAP-server.

**Standaard LDAP-wachtwoord:** als verificatie voor de LDAP-server is vereist, geeft u het wachtwoord op dat overeenkomt met de gebruikersnaam die moet worden gebruikt om verbinding te maken met de LDAP-server.
...
Met de service PDF genereren ( `GeneratePDFService`) converteert u bestanden in verschillende native indelingen naar PDF-documenten en converteert u PDF-documenten naar een aantal bestandsindelingen.
...
**Bestandstype-instellingen:** de naam van de vooraf geconfigureerde instelling voor het bestandstype die op een conversietaak moet worden toegepast, als deze instellingen niet zijn opgegeven als onderdeel van de API-oproepparameters. De instellingen voor bestandstypen worden geconfigureerd in de beheerconsole door te klikken op Services > PDF Generator > Instellingen voor bestandstypen.
...
**Reguliere expressies voor het zoeken naar verboden speciale tekens in gebruikersnaam (alleen Windows):** geeft tekens op die de Export PDF- en Optimize PDF-bewerkingen verstoren wanneer de tekens in de naam van een gebruiker worden weergegeven.

**Poolgrootte ImageToPDF:** de groepsgrootte van de standaard (zuivere Java) converter van afbeelding naar PDF in de service PDF genereren. Met deze instelling bepaalt u de maximale gelijktijdige conversies van afbeeldingen naar PDF die de service PDF genereren kan uitvoeren. De standaardwaarde van deze instelling (aanbevolen voor systemen met één processor) is 3, die u kunt verhogen voor systemen met meerdere processors.
...
**OCR-groepsgrootte:** de poolgrootte van de PaperCaptureService die door de PDF Generator wordt gebruikt voor OCR. De standaardwaarde van deze instelling (aanbevolen voor systemen met één processor) is 3, die u kunt verhogen voor systemen met meerdere processors. Deze instelling is alleen geldig op Windows-systemen.
...
**Opnieuw Logica voor native** conversiesHiermee worden nieuwe pogingen uitgevoerd voor het genereren van PDF-bestanden als de eerste poging tot conversie is mislukt:
...
**als zekerheidExpiryInSeconds:** Het interval waarna een verzoek om onderpand van een cliënt verloopt. De standaardwaarde is 315576000.
...
**guideContext:** de hoofdmap van de context van de webtoepassing Hulplijnen. Komt overeen met de waarde die is ingesteld met de webtoepassing Hulplijnen. Standaard is /Guides/.
...
**macRefreshIntervalInMinutes:** De hoeveelheid tijd dat een toets actief is. Wanneer een sleutel voor dit interval actief is geweest, wordt een nieuwe sleutel geproduceerd. De nieuwe toets wordt de actieve toets. De eerder actieve sleutel wordt bewaard voor 10% van het vernieuwingsinterval. Dit gedrag staat URLs toe die door de oude sleutel te gebruiken worden geproduceerd om over de belangrijkste schakelaar te blijven werken. De standaardwaarde is 144000.
...
**datasourceName:** Een tekenreekswaarde die de naam JNDI van de gegevensbron vertegenwoordigt om met de gegevensbestandserver te gebruiken te verbinden. De gegevensbron moet worden gedefinieerd op de toepassingsserver waarop de formulierserver wordt gehost. De standaardwaarde is de JNDI-naam van de gegevensbron voor de AEM formulierdatabase.
...
**JNDI-gebruikersnaam:** de gebruikersnaam van de account die moet worden gebruikt voor verificatie bij de JNDI-serviceprovider die wordt gebruikt voor het opzoeken van wachtrij- en onderwerpnamen. De standaardwaarde is gast.
...
**Oorspronkelijke Context Factory:** de Java-klasse die als eerste contextfabriek moet worden gebruikt. De dienst JMS gebruikt deze klasse om een aanvankelijke context tot stand te brengen, die het uitgangspunt voor het oplossen van namen van onderwerpen en rijen is. De standaardwaarde is de aanvankelijke contextfabriek voor de dienst JMS op JBoss. De volgende klassen zijn de eerste contextfabrieken voor de toepassingsservers die AEM formulieren ondersteunen:
...
**Overige eigenschappen:naam- en waardeparen van** eigenschappen die u kunt doorgeven aan het JNDI-servicebureau. Deze eigenschappen hangen van de implementatie en de configuratie van de leverancier af die u gebruikt.
...
**Oorspronkelijke Context Factory:** de Java-klasse die als contextfabriek moet worden gebruikt. Deze klasse wordt gebruikt om een verbinding met de LDAP-server te maken. De standaardwaarde is com.sun.jndi.ldap.LDAPCtxFactory, die geschikt is voor de meeste LDAP-servers.

**URL provider:** de URL waarmee verbinding wordt gemaakt met de LDAP-service. De notatie van de waarde is `ldap://server name:port`

*servernaam* is de naam van de computer die als host fungeert voor de LDAP-server

*De* communicatiepoort die de LDAP-service gebruikt. De standaardwaarde is 389. Dit is de standaardpoort die wordt gebruikt voor LDAP-verbindingen.
...
**Overige eigenschappen:** een tekenreekswaarde die andere eigenschappen vertegenwoordigt en de bijbehorende waarden die u aan de LDAP-server kunt opgeven. De waarde heeft de volgende notatie:
...
**Tijd tot live:** tijdsduur, in seconden, dat dit inrichtingsprofiel geldig is en in cache wordt geplaatst op de client. De standaardwaarde is 86400 (24 uur). Wanneer een clienttoepassing synchroniseert met de server en de opgegeven hoeveelheid tijd is verstreken, vraagt de clienttoepassing een nieuw inrichtingsprofiel aan bij de server.

**Codering:** geeft aan of gegevens die op het mobiele apparaat zijn opgeslagen, moeten worden gecodeerd.
...
**De Toepassing van taken:** laat de eigenschap van Taken in de mobiele cliënttoepassingen toe. Als deze optie is geselecteerd, kunnen gebruikers hun taaklijsten openen en taken uitvoeren vanaf hun mobiele apparaten.

**Content Services Application:** Schakelt de functie Content Services in de mobiele clienttoepassing in. Deze functie is alleen beschikbaar voor iOS. Als deze optie is geselecteerd, hebben gebruikers van de iPhone en de iPad toegang tot bestanden die zijn opgeslagen op de WebDAV-server van uw organisatie.
...
**Maximumwachtwoord voor Android wordt opnieuw gestart voor Sluitereffect:** Android-apparaten hebben een algemene instelling die het systeem wist nadat een opgegeven aantal pogingen voor een ongeldig wachtwoord zijn uitgevoerd. Deze globale instelling ingeschakeld en gelijk aan of lager dan de hier opgegeven waarde. De standaardwaarde is 5.
...
**Vereist:** ondersteunt een transactiecontext als deze al bestaat; anders wordt een nieuwe transactiecontext gecreëerd. Dit is de standaardwaarde.

**Vereist Nieuw:** maakt altijd een nieuwe transactiecontext. Als er een actieve transactiecontext bestaat, wordt deze opgeschort.

**De Tijd van de transactie uit (in seconden):** het aantal seconden dat de onderliggende transactieleverancier wacht alvorens een transactie terug te rollen die deze verrichting verpakt. Deze waarde wordt genegeerd als een bestaande transactiecontext wordt verspreid.
...
**Opstartmap van PS:** het pad van de bestandssysteemmap waarin de opstartwachtbestanden zijn opgeslagen die door Adobe Acrobat Distiller worden vereist. De standaardwaarde is [user.home]/Application Data/Adobe/Adobe PDF/Distiller/Startup.

**Opstartbestand van PS:** de naam van het opstartiebestand dat door Adobe Acrobat Distiller wordt vereist. De standaardwaarde is example.ps.

**Tijdslimiet voor serverconversie:** de maximale time-out voor taakconversie (in seconden) voor de Generate PDF-service en de Distiller-service. Met deze instelling wordt de maximale time-out voor conversie beperkt die kan worden opgegeven in het bestand config.xml en in de beheerconsolepagina&#39;s voor PDF Generator. De standaardwaarde is 270.

**Algemene time-out server:** tijdens het uitvoeren van PDF-conversies houdt een formulierserver rekening met de time-outlimiet. Configureer de time-outwaarde om het probleem op te lossen.

**Voorvoegsel taakopties:** Een voorvoegsel dat door de service PDF genereren wordt gebruikt om een korte tekenreeks voor te bereiden voor de bestanden met taakopties die tijdelijk worden gemaakt voor gebruik door Acrobat Distiller. De standaardwaarde is pdfg.
...
**Aantal threadpool van de server:** bepaalt de grootte van de draadpool die de Generate dienst PDF intern aan de dienst van HTML-aan-PDF omzettingsverzoeken gebruikt die spinnen impliceren (het omzetten van verbonden pagina&#39;s toegankelijk van de belangrijkste pagina). De standaardwaarde is 20.

**Seconden van PDFG-opschoning:** Zie de sectie Seconden van taakvervaldatums voor meer informatie.

**Seconden voor taakvervaldatum:** de service PDF genereren verwijdert invoerbestanden zodra deze worden geconverteerd. De uitvoerbestanden worden tijdelijk opgeslagen, gedurende een periode die wordt bepaald door de instellingen Seconden van PDFG-opschoning en Seconden voor taakvervaldatum.
...
De standaardwaarde van Seconden van PDFG Cleanup Scan is `43200` (12 uur). De standaardwaarde van Seconden van de Vervaltijd van de Baan is `86400` (24 uren).
...
**De Methode van de Authentificatie van de Cliënt van Flex:** bepaalt het type van reactie dat de server terug naar de cliënt verzendt wanneer de aangehaalde dienst veiligheid toegelaten is, steunt de aangehaalde verrichting geen anonieme aanroepen, en de cliënt gaat of geen of ongeldige geloofsbrieven over. Kies Aangepast of Standaard. De standaardwaarde is Standaard.

**Serienummering van niet-serialiseerbare klassen toestaan:** bij de meeste AEM eindpunten van formulieren kunnen alleen serialiseerbare klassen worden gebruikt voor aanroepen. In oudere versies, stond het Remoting eindpunt niet-Serializable klassen toe om voor aanroeping van op Flex-Gebaseerde cliënten worden gebruikt. Om een beveiligingskwetsbaarheid te voorkomen die in APS11-15 wordt beschreven, is deze kwetsbaarheid gewijzigd. Als u niet-Serializable klassen met het Flex Remoting eindpunt wilt blijven gebruiken, selecteer dit checkbox.
...
**Naam van de Verre Dienst van SPI HSM:** Deze optie is voor gebruik wanneer HSM op een verre computer wordt geïnstalleerd. Geef deze optie op wanneer AEM formulieren op een 64-bits Windows zijn geïnstalleerd en u HSM-apparaten gebruikt voor ondertekening.

**URL van de Verre Dienst van het Web HSM:** specificeer deze optie wanneer AEM vormen op Vensters met 64 bits worden geïnstalleerd en u apparaten HSM voor ondertekening gebruikt.

**Certificatie voor het opnemen van wijzigingen in het laden van formulieren:** als deze optie is geselecteerd, wordt de XFA-formulierstatus gecertificeerd naast de XFA-sjabloon. Houd er rekening mee dat het inschakelen van deze optie negatieve gevolgen kan hebben voor de prestaties. De standaardwaarde is true.

**JavaScript-scripts voor documenten uitvoeren:** geeft aan of JavaScript-scripts voor documenten moeten worden uitgevoerd tijdens handtekeningbewerkingen. De standaardwaarde is false.
...
**Ingesloten informatie over intrekking tijdens ondertekenen:** Hiermee geeft u aan of intrekkingsgegevens worden ingesloten tijdens het ondertekenen van het PDF-document. De standaardwaarde is false.

**Ingesloten informatie over intrekking bij certificering:** geeft aan of de intrekkingsinformatie wordt ingesloten tijdens certificering van het PDF-document. De standaardwaarde is false.

**Insluiten van intrekkingsinformatie voor alle certificaten tijdens ondertekenen/certificering afdwingen:** Hiermee wordt aangegeven of een ondertekenings- of certificeringsbewerking mislukt als geldige intrekkingsinformatie voor alle certificaten niet is ingesloten. Als een certificaat geen CRL- of OCSP-informatie bevat, wordt het als geldig beschouwd, zelfs als er geen intrekkingsinformatie wordt opgehaald. De standaardwaarde is false.
...
**Verificatietijd, optie:** geeft de tijd van verificatie van het certificaat van de ondertekenaar op. De standaardwaarde is Beveiligde tijd anders dan huidige tijd.

**Intrekkingsgegevens die tijdens de validatie zijn gearchiveerd met handtekening gebruiken:** Hiermee geeft u aan of de intrekkingsgegevens die met de handtekening zijn gearchiveerd, worden gebruikt voor intrekkingscontrole. De standaardwaarde is true.

**Validatie-informatie die is opgeslagen in het document gebruiken voor validatie van handtekeningen:** Als deze optie is geselecteerd, worden validatiegegevens (inclusief intrekkingsgegevens en tijdstempelgegevens) die zijn ingesloten in het document gebruikt voor de validatie van handtekeningen. De standaardwaarde is true.
...
**Maximale scheeftrekhoek voor verificatie:** de maximale tijd, in minuten, die de ondertekeningstijd na de validatietijd kan bedragen. Als de klok meer schuin is dan deze waarde, is de handtekening niet geldig. De standaardwaarde is 65 minuten.

**Certificaatlevenscache:** de levensduur van een certificaat, online of op andere wijze opgehaald, in de cache. De standaardwaarde is 1 dag.

### Vervoersopties {#transport-options}

**Proxy-host:** de URL van de proxyhost. Wordt alleen gebruikt als er een geldige waarde is opgegeven. Geen standaardwaarde.

**Proxypoort:** de proxypoort. Typ een geldig poortnummer tussen 0 en 65535. De standaardwaarde is 80.

**Gebruikersnaam inlognaam proxy:** de gebruikersnaam voor proxyaanmelding. Wordt alleen gebruikt als er een geldige waarde is opgegeven voor de proxyhost en -poort. Geen standaardwaarde.

**Wachtwoord voor aanmelding bij proxy:** het wachtwoord voor aanmelding bij proxy. Wordt alleen gebruikt als een geldige waarde is opgegeven voor de gebruikersnaam van de proxyhost, proxypoort en proxyaanmelding. Geen standaardwaarde.

**Maximale downloadlimiet:** de maximale hoeveelheid gegevens, in MB&#39;s, die per verbinding kan worden ontvangen. De minimumwaarde is 1 MB en de maximumwaarde is 1024 MB. De standaardwaarde is 16 MB.

**De Tijd van de verbinding uit:** de maximumtijd om, in seconden te wachten, voor het vestigen van een nieuwe verbinding. De minimumwaarde is 1 en de maximumwaarde is 300. De standaardwaarde is 5.

**Sockettijd uit:** De maximale tijd die in seconden moet worden gewacht voordat een sockettime-out optreedt (tijdens het wachten op gegevensoverdracht). De minimumwaarde is 1 en de maximumwaarde is 3600. De standaardwaarde is 30.

### Opties voor padvalidatie {#path-validation-options}

**Expliciet beleid vereisen:** Hiermee geeft u op of het pad geldig moet zijn voor ten minste een van de certificaatbeleidsregels die aan het vertrouwensanker van het ondertekenaarcertificaat is gekoppeld. De standaardwaarde is false.

**Willekeurig beleid onderdrukken:** Geeft aan of de OID (beleidsobject-id) moet worden verwerkt als deze in een certificaat is opgenomen. De standaardwaarde is false.
...
**Alle paden controleren:** Hiermee geeft u op of alle paden moeten worden gevalideerd of dat validatie moet stoppen nadat het eerste geldige pad is gevonden. Selecteer waar of onwaar. De standaardwaarde is false.

**LDAP-server:** de LDAP-server die wordt gebruikt om certificaten op te zoeken voor padvalidatie. Geen standaardwaarde.

**Volg URI&#39;s in Certificate AIA:** Geeft aan of Uniform Resource Identifiers (URI&#39;s) in Certificate AIA worden verwerkt tijdens paddetectie. De standaardwaarde is false.

**Basic Constraints Extension required in CA Certificates:** Geeft aan of de Certificate Authority (CA) Basic Constraints certificate extension aanwezig moet zijn voor CA-certificaten. Sommige vroege Duitse gecertificeerde basiscertificaten (7 en eerder) zijn niet compatibel met RFC 3280 en bevatten niet de basisbeperkingsextensie. Schakel dit selectievakje uit als u weet dat het EE-certificaat van een gebruiker is gekoppeld aan een dergelijk Duits basiscertificaat. De standaardwaarde is true.
...
**TSP Server-gebruikersnaam:** de gebruikersnaam indien vereist door de tijdstempelprovider. Wordt alleen gebruikt als er een geldige waarde voor de URL is opgegeven. Geen standaardwaarde.

**TSP-serverwachtwoord:** het wachtwoord voor de bovenstaande gebruikersnaam, indien vereist door de tijdstempelprovider. Wordt alleen gebruikt als er een geldige waarde voor de URL en de gebruikersnaam is opgegeven. Geen standaardwaarde.

**Hash-algoritme aanvragen:** geeft het hash-algoritme op dat moet worden gebruikt bij het maken van de aanvraag voor de tijdstempelprovider. De standaardwaarde is SHA1.

**Stijl intrekkingscontrole:** geeft de stijl van de intrekkingscontrole op die wordt gebruikt om de vertrouwensstatus van het certificaat van de tijdstempelprovider te bepalen op basis van de waargenomen intrekkingsstatus. De standaardwaarde is BestEfficient.
...
**Gebruik verlopen tijdstempels tijdens validatie:** Wanneer deze optie is geselecteerd, kunnen verlopen tijdstempels worden gebruikt om validatietijden van handtekeningen op te halen. De standaardwaarde is true.
...
**Tijdstempelserverextensie** negeren: Selecteer de optie  **TimeStamp Server-** extensie negeren om te voorkomen dat de AEM Forms-server verbinding maakt met de opgegeven tijdstempelserver. Als u deze optie selecteert, voorkomt u procesfouten die optreden als gevolg van een time-out van de verbinding tussen AEM Forms en tijdstempelservers.

### Opties voor certificaatintrekkingslijst {#certificate-revocation-list-options}

**Consult Local URI First:** Geeft aan of de CRL-locatie die in Local URI of CRL Lookup wordt aangeboden, de voorkeur moet krijgen boven elke locatie die in een certificaat is opgegeven met het oog op intrekkingscontrole. De standaardwaarde is false.

**Lokale URI voor CRL-opzoekfunctie:** URL van de lokale CRL-provider. Deze waarde wordt alleen geraadpleegd als de instelling Eerst lokale URI raadplegen is ingesteld op true. Geen standaardwaarde.

**Stijl intrekkingscontrole:** geeft de stijl van de intrekkingscontrole op die wordt gebruikt voor het bepalen van de vertrouwensstatus van het certificaat van de CRL-provider ten opzichte van de waargenomen intrekkingsstatus. De standaardwaarde is BestEfficient.

**LDAP-server voor CRL-opzoeken:** de LDAP-server die wordt gebruikt om de CRL&#39;s op te halen (als www.ldap.com). Alle op DN gebaseerde vragen voor CRLs zullen aan deze server worden geleid. Geen standaardwaarde.

**Ga online:** Geeft aan of u online wilt gaan om een CRL op te halen. Indien onwaar worden alleen in de cache opgeslagen CRL&#39;s (op de lokale schijf of op de ingesloten CRL&#39;s) geraadpleegd. De standaardwaarde is true.

**Geldigheidsdatums negeren:** Geeft aan of de reactietijden thisUpdate en nextUpdate moeten worden genegeerd, waardoor wordt voorkomen dat deze tijden een negatief effect hebben op de geldigheid van de reactie. De standaardwaarde is false.
...
**Stijl intrekkingscontrole:** geeft de stijl van de intrekkingscontrole op die wordt gebruikt tijdens de verificatie van het certificaat van de OCSP-server. De standaardwaarde is CheckIfAvailable.
...
**Maximale scheeftrektijd klok:** maximaal toegestane scheeftrekhoek in minuten tussen responstijd en lokale tijd. De minimumwaarde is 0 en de maximumwaarde is 2147483647m. De standaardwaarde is 5 m.
...
**OCSP-aanvraag ondertekenen:** geeft aan of de OCSP-aanvraag moet worden ondertekend. De standaardwaarde is false.

**Aanvraag van referentie-alias ondertekenaar:** geeft de referentie-alias op die moet worden gebruikt voor het ondertekenen van de OCSP-aanvraag als ondertekening is ingeschakeld. Wordt alleen gebruikt als de ondertekening van een OCSP-aanvraag is ingeschakeld. Geen standaardwaarde.

**Ga online:** Geeft aan of u online wilt gaan om intrekkingscontroles uit te voeren. De standaardwaarde is true.

**Negeer de tijden thisUpdate en nextUpdate van de reactie:** Specificeert of om de tijden thisUpdate en nextUpdate van de reactie te negeren, die deze tijden verhindert een negatief effect op reactiegeldigheid te hebben. De standaardwaarde is false.

**OCSPNoCheck-extensie toestaan:** Geeft aan of de OCSPNoCheck-extensie is toegestaan in het handtekeningcertificaat voor de reactie. De standaardwaarde is true.
...
**Map mislukt:** de naam van de map waarin de bestanden met fouten worden gekopieerd. Deze waarde kan een leeg, relatief of absoluut pad zijn met een bestandspatroon zoals beschreven voor de instelling Map met resultaten.
...
Als de waarde op 17 juli 2009 bijvoorbeeld 8 uur is en u `C:/Test/WF0/failure/%Y/%M/%D/%H/` opgeeft, is de resultaatmap `C:/Test/WF0/failure/2009/07/17/20`.

Als het pad niet absoluut maar relatief is, wordt de map in de controlemap gemaakt. Zie [Informatie over bestandspatronen](/help/forms/using/admin-help/configuring-watched-folder-endpoints.md#about-file-patterns) voor meer informatie over bestandspatronen.
...
**Behouden bij fout:** Indien waar (true), blijven de oorspronkelijke bestanden behouden in de map met mislukkingen.
...
**sleutelarchief:** het volledige pad van het sleutelarchiefbestand dat de persoonlijke sleutel bevat die voor verificatie moet worden gebruikt. De formulierserver moet toegang hebben tot het bestand.

**Wachtwoord sleutelarchief:** het wachtwoord voor het sleutelarchiefbestand.

**Type sleutelarchief:** het type sleutelarchief. Geef geen waarde op om het standaardsleutelarchieftype te gebruiken dat is geconfigureerd voor de JVM die de formulierserver uitvoert. Geef anders een van de volgende waarden op:
...
**Trust Store:** het volledige pad van het vertrouwde opslagbestand dat de openbare sleutel van de webserviceserver bevat.

**Wachtwoord voor winkel vertrouwen:** het wachtwoord voor het bestand truststore.

**Winkeltype vertrouwen:** het type vertrouwde opslag. Geef geen waarde op om het standaardsleutelarchieftype te gebruiken dat is geconfigureerd voor de JVM die de formulierserver uitvoert. Geef anders een van de volgende waarden op:
