...
## <strong>Een adaptief formulier</strong> maken {#strong-create-an-adaptive-form-strong}
...
1. Voer uw referenties in op de AEM-aanmeldingspagina.
   ...
   Tik in de linkerbovenhoek op **[!UICONTROL Adobe Experience Manager > Formulieren > Formulieren en documenten]** nadat u zich hebt aangemeld.

   >[!NOTE]
   >
   >Voor een standaardinstallatie, login is `admin` en het wachtwoord is `admin`.

1. Tik op **[!UICONTROL Maken]** en selecteer **[!UICONTROL Adaptief formulier]**.
...
1. U kunt een adaptief formulier maken op basis van een van de volgende formuliermodellen:
   ...
   * Geen of geen formuliermodel
   U kunt deze configureren op het tabblad **[!UICONTROL Formuliermodel]** op de pagina Eigenschappen **** toevoegen. Standaard is het geselecteerde formuliermodel **[!UICONTROL Geen]**.
...
1. Tik op **Maken**. Er wordt een adaptief formulier gemaakt en er wordt een dialoogvenster weergegeven om het formulier te openen voor bewerking.
   ...
   Als u alle eigenschappen hebt opgegeven, klikt u op **[!UICONTROL Maken]**. Er wordt een adaptief formulier gemaakt en er wordt een dialoogvenster weergegeven om het formulier te openen voor bewerking.

   Als u alle eigenschappen hebt opgegeven, klikt u op **[!UICONTROL Maken]**. Er wordt een adaptief formulier gemaakt en er wordt een dialoogvenster weergegeven om het formulier te openen voor bewerking.

1. Tik op **[!UICONTROL Openen]** om het nieuwe formulier te openen op een nieuw tabblad. Het formulier wordt geopend voor bewerking en geeft de inhoud weer die beschikbaar is in de sjabloon. De zijbalk wordt ook weergegeven om het nieuwe formulier aan te passen aan de behoeften.

   Op basis van het type adaptief formulier worden de formulierelementen in de bijbehorende XFA-formuliersjabloon, het XML-schema of het JSON-schema weergegeven op het tabblad **[!UICONTROL Gegevensmodelobjecten]** van de **[!UICONTROL Inhoudsbrowser]** in het zijpaneel. U kunt deze elementen ook slepen en neerzetten om het aangepaste formulier samen te stellen.
...
[Met AEM Forms kunt u meerdere gegevensbronnen integreren en hun entiteiten en services samenbrengen om een formuliergegevensmodel te maken.](/help/forms/using/data-integration.md) Het is een uitbreiding van het JSON-schema. U kunt een formuliergegevensmodel gebruiken om een adaptief formulier te maken. De entiteiten of gegevensmodelobjecten die in een formuliergegevensmodel zijn geconfigureerd, zijn beschikbaar als gegevensmodelobjecten voor het ontwerpen van formulieren. Zij zijn gebonden aan de respectieve gegevensbronnen en worden gebruikt om een formulier vooraf in te vullen en ingediende gegevens terug te schrijven naar de respectieve gegevensbronnen. U kunt services die zijn geconfigureerd in een formuliergegevensmodel ook aanroepen met behulp van adaptieve formulierregels.
...
1. Open op de pagina Eigenschappen **** toevoegen het tabblad **[!UICONTROL Formuliermodel]** .
...
1. Tik op het tabblad **[!UICONTROL Formuliermodel]** op de stap Eigenschappen **** toevoegen van de aangepaste pagina voor het maken van formulieren.
1. Selecteer op het tabblad Formuliermodel de optie **[!UICONTROL Schema]** in het vervolgkeuzemenu **[!UICONTROL Selecteren uit]** .

1. Tik op Schema **** selecteren en voer een van de volgende handelingen uit:

   * **[!UICONTROL Uploaden vanaf schijf]** - Selecteer deze optie en tik op Schemadefinitie uploaden om te bladeren naar en een XML-schema of JSON-schema te uploaden vanuit uw bestandssysteem. Het geüploade schemabestand bevindt zich bij het formulier en is niet toegankelijk voor andere adaptieve formulieren.
   * **[!UICONTROL Zoeken in opslagplaats]** - Selecteer deze optie om een keuze te maken uit de lijst met schemadefinitiebestanden die beschikbaar zijn in de opslagplaats. Selecteer het XML- of JSON-schemabestand als formuliermodel. Het geselecteerde schema wordt via verwijzing aan het formulier gekoppeld en is toegankelijk voor gebruik in andere adaptieve formulieren.
...
1. Open het tabblad **[!UICONTROL Formuliermodel]** en voer een van de volgende handelingen uit.
...
1. Tik op **[!UICONTROL Opslaan]** om de eigenschappen op te slaan.
