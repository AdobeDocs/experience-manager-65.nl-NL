...
### Tekst {#create-text} maken

1. Selecteer **Forms** > **Documentfragmenten**.
1. Tik **Maken** > **Tekst** of selecteer een tekstelement en tik **Bewerken**.
1. Geef de volgende informatie op voor de tekst:

   * **Titel: (Optioneel)** Voer de titel voor het tekstelement in. Titels hoeven niet uniek te zijn en kunnen speciale tekens en niet-Engelse tekens bevatten. De teksten worden bedoeld door hun titels (indien beschikbaar) zoals in duimnagels en activa eigenschappen.
...
1. Tik **Volgende**. Met Correspondentiebeheer wordt de Editor-pagina weergegeven waar u tekstalinea&#39;s en gegevenselementen aan de tekst kunt toevoegen.
...
1. Er wordt een teksteditor geopend en u voert de tekst in. Gebruik de werkbalk boven aan de pagina om de tekst, invoegvoorwaarden, koppeling en pagina-einden op te maken.
   ...
   * **Koppeling**: Voeg  [](#insert-hyperlink) hypertextlink in de tekst in.
...
1. Voeg zo nodig speciale tekens in het documentfragment in. U kunt bijvoorbeeld het palet Speciale tekens gebruiken om het volgende in te voegen:
   ...
   * Interpunctiesymbolen zoals ‟ en&quot;
   ...
   Correspondence Management biedt ondersteuning voor 210 speciale tekens. De beheerder kan [ondersteuning toevoegen voor meer/aangepaste speciale tekens door aanpassing](/help/forms/using/custom-special-characters.md).
...
1. U kunt inline voorwaarden en herhalingen gebruiken om uw brief in hoge mate contextueel en goed gestructureerd te maken. Zie [Inline voorwaarden en herhaal deze in letters](/help/forms/using/cm-inline-condition.md) voor meer informatie over inline voorwaarde en herhaling.
1. Tik **Opslaan**.

#### Hyperlink invoegen in een tekst {#insert-hyperlink}
...
2. Tik op **[!UICONTROL Link]**. Tik op het veld **[!UICONTROL Alt Text]** om de bestaande naam of tekst van het gegevensmodel te verwijderen.

3. Geef de URL op en tik ![Opslaan](assets/save_icon.svg).

![Hyperlink maken in tekstelement](assets/text-create-hyperlink.png)

#### Tekst {#searching-and-replacing-text} zoeken en vervangen
...
#### Tips en trucs voor beste praktijken/voor tekstmodules {#best-practices-tips-and-tricks-for-text-modules}
...
* De volgende regels zijn van toepassing wanneer u de Teksteditor gebruikt bij het wijzigen van een tekstelement:
   ...
   * **Wijzigen van gegevenswoordenboek:** toegestaan totdat gegevenswoordenboekelement niet wordt gebruikt. U kunt het gegevenswoordenboek niet wijzigen tijdens het bijwerken.
...
### Implementatielijsten {#implementing-lists}
...
1. U kunt de volgende opties selecteren om op te geven hoe elk element in de lijst zich gedraagt bij uitvoering:
   ...
   * **Verplicht:** Wanneer deze optie is geselecteerd, is de inhoud vereist in de gebruikersinterface Correspondentie maken.
   * **Geselecteerd:** Wanneer deze optie is geselecteerd, wordt de inhoud vooraf geselecteerd in de gebruikersinterface Correspondentie maken.
   * **Stijl overslaan:** als deze optie is geselecteerd, worden opsommingstekens en nummering in de gebruikersinterface Correspondentie maken overgeslagen. (Deze optie is niet beschikbaar voor afbeeldingsmodules. Tussen Stijl, Samenstelling en Lijststijl negeren kan bovendien slechts een van de opties worden toegepast op een module. Een van deze opties kan voor een module worden gebruikt wanneer u Opsommingstekens toevoegen voor een module selecteert.)
   ...
   * **Samengesteld:** Wanneer deze optie is geselecteerd, wordt de samengestelde nummering toegepast als een combinatie van de stijl van de buitenste (bovenliggende) lijst en de stijl ervan is een eigen stijl. De samengestelde nummering in deze geneste lijst is gebaseerd op de volgorde waarin deze geneste lijst wordt weergegeven in de buitenste lijst.
   ...
   * **Houden bij volgende:** Hiermee stelt u het pagina-einde in voor de elementen in een lijst. Als u de eigenschap Houden bij volgende van een element in een lijst instelt op **Aan**, blijven dat element en het volgende element op dezelfde pagina. Dit houdt in dat de inhoud van het geselecteerde element en het volgende element niet worden verdeeld over pagina&#39;s.
...
* De volgende regels zijn van toepassing wanneer u de List Editor gebruikt om een lijst te wijzigen:
   ...
   * Bijwerken van eigenschappen: Toegestaan
   * **Wijziging in gegevenswoordenboek:** Toegestaan totdat er geen item is gekoppeld dat het gegevenswoordenboek gebruikt. U kunt het gegevenswoordenboek niet wijzigen tijdens het bijwerken.
...
1. Tik **Delete** om de rij te verwijderen. Als u op Verwijderen tikt voor de standaardrij, worden alleen de elementgegevens gewist.
1. Tik **Kopieer** om een rij te dupliceren.
1. Tik op **Bewerken** om het element te wijzigen of de expressie te bewerken.
   ...
   * Tik op het mappictogram onder de kolom Expressie om de expressiebouwer te openen en een expressie in te voegen. Voor meer informatie over de Bouwer van de Uitdrukking, zie [de Bouwer van de Uitdrukking](/help/forms/using/expression-builder.md).
...
* De volgende regels zijn van toepassing wanneer u de Condition Editor gebruikt om een voorwaarde te bewerken:
   ...
   * **Verwijderen van variabele:** Toegestaan
   * **Bijwerken van eigenschappen:** toegestaan
   * **Wijzigen van gegevenswoordenboek:** toegestaan totdat gegevenswoordenboekelement niet wordt gebruikt.

## Layoutfragmenten {#layoutfragments}

Een lay-outfragment is gebaseerd op XDPs die in Ontwerper worden gecreeerd. Als u lay-outfragmenten wilt maken, moet u de XDP&#39;s maken en [deze uploaden naar AEM Forms](/help/forms/using/import-export-forms-templates.md).
...
Zie [Voorbeeld met voorbeeldbestanden voor een gedetailleerd voorbeeld van het gebruik van statische en dynamische tabellen in lay-outfragmenten: het gebruiken van statische en dynamische lijsten in een brief](#examplewithsamplefiles).

### Een lay-outfragment {#creating-a-layout-fragment} maken

1. Selecteer **Maken** > **Lay-outfragment**.
1. Correspondence Management geeft de beschikbare XDP&#39;s weer. Selecteer XDP waarop u uw lay-outfragment wilt baseren en **Volgende** tikken.
1. Geef de volgende informatie op voor de indeling:

   * **Titel (optioneel):** voer de titel voor het layoutfragment in. De titel mag niet uniek zijn en mag speciale tekens en niet-Engelse tekens bevatten. Indelingsfragmenten worden aangeduid met hun titels (indien beschikbaar), zoals miniaturen en elementeigenschappen.
...
Zie [Elementen importeren en exporteren naar AEM Forms](/help/forms/using/import-export-forms-templates.md) voor instructies voor het uploaden/importeren van een XDP naar Correspondence Management.

### Tips en trucs {#best-practices-tips-and-tricks-2}

#### De standaardbinding voor het subformulier instellen {#set-the-default-subform-binding}
...
#### Sectie 508-compatibiliteit {#section-compliance}
...
Zorg er tijdens het ontwerpen van een lay-out in Designer voor dat de indelingen voor datumvelden overeenkomen met de datumnotaties die zijn opgegeven in Gegevensweergave-indelingen in [Eigenschappen van Correspondentiebeheer](/help/forms/using/cm-configuration-properties.md). Zie &quot;Veldwaarden opmaken en patronen gebruiken&quot; in de Help van Designer voor meer informatie.

#### Datumbereiken {#capturing-date-ranges} vastleggen
...
#### Doelen van subformulierdoelgebieden op een master pagina {#do-not-use-subform-target-areas-in-a-master-page} niet gebruiken
...
#### Aangepaste posities en typen kiezen voor doelgebieden {#choosing-appropriate-positions-and-types-for-target-areas}
...
## Tabellen maken met layoutfragmenten {#creating-tables-using-layout-fragments}
...
### Voorbeeld met voorbeeldbestanden: Statische en dynamische tabellen gebruiken in een letter {#examplewithsamplefiles}
...
1. Maak een gegevenswoordenboek (DD) dat u in het voorbeeld wilt gebruiken, zoals wordt weergegeven in de afbeelding.
   ...
   Zie [Gegevenswoordenboek](/help/forms/using/data-dictionary.md#p-working-with-test-data-p) voor meer informatie over het werken met gegevenswoordenboek en testgegevens.
...
1. Maak in Designer twee XDP&#39;s (layoutfragmenten): een dynamische tabel en een statische tabel. Voor beide lay-outs:
   ...
   * Voeg een subformulier toe aan de tabelcel. Zorg ervoor dat u de indeling van het bovenliggende subformulier van de tabel wijzigt in Stroominhoud en dat u de bindingen van het subformulier uit de tabel verwijdert.
   ...
   Zie [Lay-outfragmenten](#layoutfragments) voor meer informatie over het werken met lay-outfragmenten.
Zie [Help bij Designer](https://help.adobe.com/en_US/AEMForms/6.1/DesignerHelp/) voor meer informatie over het ontwerpen van lay-outs.
...
1. Maak een letter met beide lay-outfragmenten erin. Wanneer u dynamische XDP in de brief opneemt, plaats de band van zijn gebieden aan de de inzamelingselementen van het Woordenboek van Gegevens.
   ...
   Zie [Letter maken](/help/forms/using/create-letter.md) voor meer informatie over het maken van letters en lettertypesjablonen.
...
## Een kopie maken van een documentfragment {#create-a-copy-of-a-document-fragment}
