...
description: Leer hoe te opstelling een project van Visual Studio om AEM de mobiele apparatenapp van Vensters van Vormen Windows te bouwen.
seo-description: Leer hoe te opstelling een project van Visual Studio om AEM de mobiele apparatenapp van Vensters van Vormen Windows te bouwen.
...
AEM Forms bevat de volledige broncode van de app AEM Forms. De bron bevat alle componenten om een toepassing van de douanewerkruimte te bouwen. Het broncodearchief `adobe-lc-mobileworkspace-src-<version>.zip`maakt deel uit van het `adobe-aemfd-forms-app-src-pkg-<version>.zip` pakket voor delen van pakketten.
...
## Het Project van Visual Studio van vestiging voor app van Vormen AEM {#setting-up-visual-studio-project-for-aem-forms-app}

Voer de volgende stappen aan opstelling uit AEM vormt app project in Visual Studio.
...
1. Navigeer naar de `%HOMEPATH%\Projects\MobileWorkspace\adobe-lc-mobileworkspace-src-[versionsrc]\windows` map.
1. Open het `CordovaApp.sln` dossier gebruikend Visual Studio 2015 en ga aan de bouw van AEM te werk vormt app.

## AEM Forms-app ontwikkelen {#build-aem-forms-app}

Voer de volgende stappen uit om de app AEM Forms te maken en te implementeren.

>[!NOTE]
>
>Gegevens die in Windows-bestandssysteem voor de app AEM Forms zijn opgeslagen, worden niet gecodeerd. Men adviseert dat u een derdehulpmiddel zoals de Encryptie van de Aandrijving van Windows BitLocker gebruikt om schijfgegevens te coderen.
