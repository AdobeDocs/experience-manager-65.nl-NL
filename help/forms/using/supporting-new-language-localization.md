...
AEM Forms ondersteunt momenteel de lokalisatie van adaptieve formulieren met inhoud in het Engels (en), Spaans (es), Frans (fr), Italiaans (it), Duits (de), Japans (ja), Portugees-Braziliaans (pt-br), Chinees (zh-tn), Chinees-Taiwan (zh-tw) en Koreaans (ko-kr).
...
Voer deze stap alleen uit als de stap die `<locale>` u toevoegt, niet behoort tot `en`, `de`, `es`, `fr`, `it`, `pt-br`, `zh-tn`, `zh-tw`, `ja``ko-kr`.
...
1. Voeg de `<locale>` standaardwaarden voor de landinstelling toe `de`, `es`, `fr`, `it`, `pt-br`, `zh-tn`, `zh-tw`, `ja`, `ko-kr`als deze nog niet aanwezig zijn.
