...
description: Volg deze pagina voor meer informatie over Content Sync voor Adobe PhoneGap Enterprise met AEM.
seo-description: Volg deze pagina voor meer informatie over Content Sync voor Adobe PhoneGap Enterprise met AEM.
...
>[!NOTE]
>
>PhoneGap-apps die u met AEM-gereedschappen maakt, zijn al geconfigureerd om AEM-pagina&#39;s als inhoud te gebruiken via Content Sync.
...
>[!NOTE]
>
>Zie Handlers voor [inhoudssynchronisatie ontwikkelen voor meer informatie over richtlijnen voor het ontwikkelen van handlers voor inhoudssynchronisatie uit de box app handlers](/help/mobile/contentsync-app-handlers.md).
...
Als het `cq:ContentSyncConfig` knooppunt geen autoriseerbare eigenschap heeft, bepaalt de standaardgebruiker of -groep die is geconfigureerd voor de eigenschap Fallback Cache Authorizable van de Day CQ Content Sync Manager-service wie kan downloaden.
...
Als de knoop cq:ContentSyncConfig geen updateuser bezit heeft, werkt de standaard anonieme gebruiker het geheime voorgeheugen bij.
...
**inhoud** Inhoud renderen met gebruik van de standaardverwerking van een sorteeraanvraag.
...
* **includeFolders** - Type is een array van tekenreeksen en de gebruiker kan aanvullende mappen opgeven die in de clientbibliotheek moeten worden gescand om bestanden op te halen (zoals aangepaste lettertypen).
...
* **renditions** - Type is een array van tekenreeksen, waarmee de gebruiker kan opgeven welke vertoningen moeten worden gebruikt in plaats van de standaardafbeelding. De volgende lijst geeft een overzicht van enkele uitvoeringen buiten de box, maar u kunt ook elke uitvoering gebruiken die door de workflow wordt gemaakt:
