...
Het nieuwe protocolDASH (Dynamische Aanpassings het Streamen over HTTP) steunt die voor adaptieve bitrate het stromen in Dynamische Video van Media wordt gelanceerd (met CMAF [ toegelaten het Gemeenschappelijke Formaat van de Toepassing van Media ]).
...
* Op JBoss® 7.1.4-platform mislukt de implementatie van `adobe-livecycle-jboss.ear` wanneer een gebruiker AEM 6.5.16.0 Service Pack installeert.
