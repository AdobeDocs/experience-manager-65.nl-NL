...
* **Naadloze compatibiliteit met Windows 11 voor Forms Designer**: AEM Forms Designer biedt nu ondersteuning voor Windows 11 en zorgt voor een vloeiende installatie en werking. Gebruikers kunnen met vertrouwen een upgrade naar Windows 11 uitvoeren zonder dat ze zich zorgen hoeven te maken over de herinstallatie van Forms Designer of over compatibiliteitsproblemen, waardoor een ononderbroken workflow wordt gegarandeerd.
...
* Nadat een PDF naar een andere plaats wordt verplaatst, **[!UICONTROL View Page]** verdwijnt. (ASSETS-30538)
...
* Wanneer een element naar AEM wordt geüpload, `Update_asset` de workflow wordt geactiveerd. De workflow wordt echter nooit voltooid. De workflow wordt alleen voltooid tot de uploadprocedure voor het product. De volgende stap is de Scene7 batch upload, maar dat proces wordt niet in AEM gehaald. (ASSETS-30443)
* U hebt een betere manier nodig om niet-Dynamic Media video&#39;s netjes af te handelen in de Dynamic Media-component. Deze kwestie gaf een uitzondering concretiseren `dynamicmedia_sly.js`. (ASSETS-31301)
* Voorvertonen werkt voor alle elementen, adaptieve videosets en video&#39;s. Er treedt echter een fout van 403 op voor `.m3u8` bestanden (die overigens nog steeds via openbare koppelingen werken). (ASSETS-31882)
* De `scene7SmartCropProcessingStatus` status gecorrigeerd. Metagegevens van video voor SmartCrop die worden gebruikt om een fout weer te geven, zelfs wanneer dit gelukt was. (ASSETS-31255)
...
* Op AEM Forms Designer geïnstalleerd via Workbench zijn de versiegegevens niet expliciet vermeld in het dialoogvenster `Control Panel/Programs/Programs and Features`. (LC-392/1976)
