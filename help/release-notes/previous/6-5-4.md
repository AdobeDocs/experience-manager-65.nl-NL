...
* U kunt selectieve inhoud nu synchroniseren onderaan aan *Dynamische Media - wijze Scene7* in plaats van allen beschikbaar bij `content/dam`.

* Integratie van het formuliergegevensmodel met de SOAP-webservice ondersteunt nu keuzegroepen of kenmerken voor elementen.

* SOAP-invoer of -uitvoer en complexe gegevensstructuren bieden nu ondersteuning voor Dynamic Group Substitution.
...
* Wanneer u de component Drag hier optie in ontvankelijke net placeholder selecteert, wordt een verzoek van GET verzonden en het verzoek resulteert in fout HTTP 403 (NPR-31845).
...
* Een omslag zonder naam wordt gecreeerd in SPS (het Publiceren Scene7 Systeem) terwijl het bewegen van activa van één omslag aan een andere in Experience Manager met Dynamische Configuratie van Media Scene7 (NPR-32440).
...
* De Assets detailpagina van de activa van PDF toont actieknopen behalve aan Inzameling en voegt de knopen van de Vertoning in Experience Manager toe die op Dynamische Media Scene7 looppas wijze lopen (CQ-4286705).

* Assets duurt te lang om het batchuploadproces van Scene7 (CQ-4286445) te verwerken.
...
* De Carousel Editor wordt niet geopend in IE 11 in de modus Experience Manager Dynamic Media Hybrid (CQ-4255590). **voor de Dynamische Media van Adobe - Hybride wijzeklanten:** Adobe beëindigt steun voor Internet Explorer 11 op Dynamische Media - Hybride wijze, na Mei 2022.
...
* Het selectievakje Alle inhoud synchroniseren is niet standaard ingeschakeld wanneer wordt geprobeerd om een DM-cloud te configureren in Experience Manager (CQ-4288533).
...
* MS Word- en MS Excel-documenten met JPEG-afbeeldingen kunnen niet worden geparseerd en er is een fout opgetreden bij het parseren van de klasse. (NPR-31952).
...
* Document Services: als het aantal transacties op een server een specifieke limiet overschrijdt, mislukt de conversie van HTML naar PDF en worden de instellingen voor het bestandstype verwijderd van de [!DNL Forms] server (NPR-32204).
...
* Workbench: er wordt een foutbericht weergegeven wanneer u de bewerking PDF-eigenschappen ophalen voor de PDF Utilities-service (NPR-32150) gebruikt.
...
* Designer: Als de optie Tags toevoegen is ingeschakeld, verdwijnt de rand van het subformulier in de gegenereerde PDF-uitvoer (NPR-32547, NPR-31983, NPR-31950).
