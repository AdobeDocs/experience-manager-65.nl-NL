...
* De standaardopties voor `test` en `aiprocess` worden bijgewerkt naar `Thumbnail` , vanaf `Rasterize` eerder in Dynamic Media, om ervoor te zorgen dat gebruikers alleen miniaturen hoeven te maken en de extractie van pagina&#39;s en trefwoorden moeten overslaan.
...
* Steun voor [&#x200B; de Markeringen van de Tekst van het Teken van Adobe in de Geautomatiseerde Dienst van de Omzetting van Vormen &#x200B;](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/convert-existing-forms-to-adaptive-forms.html).

* Steun om [&#x200B; gekleurde vormen in adaptieve vormen &#x200B;](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/convert-existing-forms-to-adaptive-forms.html) om te zetten gebruikend [!DNL Automated Forms Conversion service].

* Steun voor SMB 2 en SMB 3 protocollen.

* De ingebouwde opslagplaats (Apache Jackrabbit Oak) wordt bijgewerkt naar versie 1.2.4.

Hier volgt een lijst met oplossingen die worden aangeboden in [!DNL Experience Manager] 6.5.6.0 -release.
...
* Na de upgrade van Experience Manager 6.3.3 naar Experience Manager 6.5.3 duurt het uitrollen veel langer en mislukt de uitrol met een time-outfout (NPR-34049).
...
* Het uitnemen van tekst en dus het zoeken naar de geüploade PDF-bestanden in de volledige tekst werkt niet (NPR-34164). Start de implementatie van [!DNL sAdobe Experience Manager] opnieuw nadat u Service Pack 6 hebt geïnstalleerd om dit probleem op te lossen.
...
**Vaste Kwesties in Dynamische Media**
...
* Als u een eigenschap selecteert terwijl u een Adobe Launch-configuratie maakt, wordt het volgende foutbericht weergegeven (NPR-33947):
...
* In de beschrijving van de Help-inhoud van een adaptief formulierveld wordt een HTML-alineatag (NPR-34116) weergegeven.
...
* Wanneer u het Pak van de Dienst van AEM installeert, **om** lijst van punten te doen toont niet als verbindingen. De tekst voor **om** punten te doen omvat de markeringen van HTML (NPR-34317).
...
* Wanneer u een HTML-bestand converteert naar PDF, kan het kenmerk `MaxReuseCount` niet worden geconfigureerd (NPR-33470).

* Wanneer u een PDF-bestand downloadt dat interactieve functies voor Reader Extensions bevat, kunt u geen bijlage aan het PDF-bestand toevoegen met [!DNL Adobe Reader] (NPR-33729).
