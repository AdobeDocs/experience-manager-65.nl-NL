...
Hier volgt een lijst met oplossingen die worden aangeboden in [!DNL Experience Manager] 6.5.8.0 -release.
...
* Wanneer u SSL inschakelt voor een Experience Manager-instantie, wordt het contextpad verwijderd (NPR-35477).
...
* Wanneer u een AEM-instantieupgrade start van AEM 6.3 naar AEM 6.5, duurt het langer om de upgrade-instantie te starten (NPR-35323).
* Wanneer u een AEM-element repliceert dat een haakje () bevat. in de naam, ontbreekt de replicatie (GRANITE-27004, NPR-35315).
...
* Wanneer minificatie is ingeschakeld, kunt u geen enkele pagina of component bewerken. De problemen zijn begonnen na de installatie van AEM 6.5 Service Pack 7 (CQ-4311133).
...
* Voor geüploade elementen die automatisch naar DM worden gepubliceerd, wordt de status Gepubliceerd op AEM niet weergegeven. Bovendien wordt in de statuskolom Dynamische media publiceren niet de juiste gepubliceerde status weergegeven (CQ-4306415).
...
* Nadat u hebt geüpgraded naar AEM 6.5.7, mislukken de builds. De reden hiervoor is dat een oude versie of geen cola van de nakker is ingebed in de uber-jar (GRANITE-33006).
...
* De PDF forms die wordt gegenereerd met behulp van de Output Service omvat geen randen (CQ-4312564).
...
* Als u een upgrade uitvoert van de [!DNL Experience Manager] -instantie van 6.5 naar de 6.5.8.0 -versie, kunt u `RRD4JReporter` -uitzonderingen weergeven in het `error.log` -bestand. Start de instantie opnieuw om het probleem op te lossen.
* Neem contact op met de klantenondersteuning van Adobe als u problemen ondervindt bij het bewerken en maken van trapsgewijze regels in het dialoogvenster [!UICONTROL Folder Metadata Schema Forms Editor] en [!UICONTROL Metadata Schema Forms Editor] gebruiken [!UICONTROL Define Rule] . De regels die al zijn gemaakt en opgeslagen, werken zoals u had verwacht.
