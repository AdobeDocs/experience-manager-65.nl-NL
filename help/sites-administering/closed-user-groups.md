...
Raadpleeg de [&#x200B; documentatie 0&rbrace; van de PAGINA pluggability &lbrace;voor details op hoe te om het de vergunningsmodel van de KUG van de samengestelde vergunningsopstelling te verwijderen.](https://jackrabbit.apache.org/oak/docs/security/authorization/cug.html#pluggability)
...
Het plaatsen van het beleid van de GIDS slechts bij de knoop jcr :content die onder een bepaalde pagina wordt gevestigd beperkt slechts toegang tot de inhoud s.str van een bepaalde pagina maar zal niet op om het even welke siblings of kindpagina&#39;s van kracht worden. Dit kan een geldig gebruiksgeval zijn en het is mogelijk om met een bewaargegevensopslagredacteur te bereiken die u fijne korrelige toegangsinhoud laat toepassen. Nochtans, contrasteert het de vroegere implementatie waar het plaatsen van een cq :cugEnabled bezit op jcr :content knoop intern aan de paginaknooppunt werd opnieuw in kaart gebracht. Deze toewijzing wordt niet meer uitgevoerd.
...
Het plaatsen van het beleid van de GIDS slechts bij de jcr :content knoop die onder een bepaalde pagina wordt gevestigd beperkt slechts toegang tot de inhoud. Het heeft echter geen invloed op het paginaknooppunt zelf of op onderliggende pagina&#39;s.

Dit kan een geldig scenario zijn en is mogelijk met een bewaargegevensverwerker die u de mixin bij om het even welk knooppunt laat plaatsen. Nochtans, contrasteert het gedrag de vroegere implementatie, waar het plaatsen van een cq :cugEnabled of cq :cugLoginPage bezit op jcr :content knoop intern werd opnieuw in kaart gebracht uiteindelijk aan de paginaknooppunt. Deze toewijzing wordt niet meer uitgevoerd.

#### Geconfigureerde ondersteunde paden {#configured-supported-paths}

Zowel zal het `granite:AuthenticationRequired` mixintype als graniet :loginPath bezit slechts binnen het werkingsgebied worden gerespecteerd dat door de reeks **wordt bepaald gesteunde de configuratieoptie van Wegen** huidig met de **Vereiste van de Authentificatie van de Graniet van Adobe en de Handler van de Weg van de Login**. Als er geen paden zijn opgegeven, is de functie voor de verificatie-eisen volledig uitgeschakeld. In dit geval wordt het mixintype en de eigenschap van kracht wanneer het wordt toegevoegd aan of ingesteld op een bepaald JCR-knooppunt.
