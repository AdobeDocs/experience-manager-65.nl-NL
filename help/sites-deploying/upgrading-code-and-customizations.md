...
1. **AEM-aanpassingen*** bijwerken - *Aanpassingen of uitbreidingen van AEM moeten worden bijgewerkt/gevalideerd om in versie 6.5 te kunnen werken en aan de basis van de 6.5-code worden toegevoegd. Bevat UI Search Forms, Assets Customizations, alles wat gebruikmaakt van /mnt/overlay

1. **Implementeren in 6.5-omgeving** - Een schoon exemplaar van AEM 6.5 (Auteur + Publiceren) moet in een Dev/QA-omgeving staan. De bijgewerkte codebasis en een representatieve steekproef van inhoud (van huidige productie) zouden moeten worden opgesteld.
...
Naast de mogelijkheid om uw codebasis en aanpassingen bij te werken met de nieuwe AEM-versie, helpt 6.5 ook om uw aanpassingen efficiënter te beheren met de functie Achterwaartse compatibiliteit die op [deze pagina](/help/sites-deploying/backward-compatibility.md)wordt beschreven.

Zoals hierboven vermeld en in het diagram hieronder getoond, zal het runnen van de Detector [van het](/help/sites-deploying/pattern-detector.md) Patroon in de eerste stap u helpen de algemene ingewikkeldheid van de verbetering beoordelen en of u op verenigbaarheidswijze wilt lopen of uw aanpassingen bijwerken om alle nieuwe eigenschappen te gebruiken AEM 6.5. Zie de pagina [Achterwaartse compatibiliteit in AEM 6.5](/help/sites-deploying/backward-compatibility.md) voor meer informatie.
...
* [Eik](https://oakutils.appspot.com/). Dit is een opensource-programma dat niet door Adobe wordt beheerd.

### Klassieke UI Authoring {#classic-ui-authoring}

Klassieke UI-authoring is nog steeds beschikbaar in AEM 6.5, maar wordt afgekeurd. Meer informatie vindt u [hier](/help/release-notes/deprecated-removed-features.md#pre-announcement-for-next-release). Als uw toepassing momenteel op het Klassieke auteursmilieu van UI loopt, wordt het geadviseerd om aan AEM 6.5 te bevorderen en verder gebruikend Klassieke UI te blijven. De migratie naar Touch UI kan dan als afzonderlijk project worden gepland dat over verscheidene ontwikkelingscycli wordt voltooid. Om Klassieke UI in AEM 6.5 te gebruiken zijn verscheidene configuraties OSGi nodig om aan de codebasis worden geëngageerd. Meer details over hoe te om dit te vormen kunnen [hier](/help/sites-administering/enable-classic-ui.md)worden gevonden.
...
Daarom moet een aantal instellingen worden verplaatst om niet langer te verblijven, `/etc` zoals in het verleden het geval was geweest. Zie [Repository Reform in AEM 6.4](/help/sites-deploying/repository-restructuring-in-aem65.md)voor een overzicht van de volledige reeks herstructureringsproblemen in de opslagplaats die in de bijgewerkte versie naar AEM 6.4 moeten worden herzien en opgenomen.
...
De configuraties ContextHub worden uitgevoerd door een verbetering. De instructies op hoe te om bestaande configuraties terug te krijgen ContextHub kunnen [hier](/help/sites-administering/contexthub-config.md#het terugkrijgen van contexthub configuraties na bevordering) worden gevonden.
...
Hieronder vindt u een aantal kritieke gebieden van een AEM-implementatie die onder uw testplan moeten vallen zodra de omgeving is bijgewerkt en de geüpgrade codebasis is geïmplementeerd.
...
<table>
 <tbody>
  ...
  <tr>
   <td>Authoring</td>
   <td>Testen van de AEM implementatie en bijbehorende code op de laag Auteur. Dit moet pagina, componentontwerp en dialoogvensters bevatten.</td>
  </tr>
  <tr>
   <td>Integratie met Marketing Cloud Solutions</td>
